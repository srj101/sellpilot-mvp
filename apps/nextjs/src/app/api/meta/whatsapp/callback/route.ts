...
export async function GET(request: Request) {
  ...
  // Not under /{businessSlug}/dashboard/*, so it relies on the same
  // meta_channel_store_slug cookie stashed by connectChannel (see
  // /api/meta/callback/route.ts for the full explanation).
  const businessSlug = (await cookies()).get("meta_channel_store_slug")?.value;
  const destination = new URL(
    businessSlug ? `/${businessSlug}/dashboard/integrations` : "/dashboard",
    url.origin,
  );
