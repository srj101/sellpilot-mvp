...
export function InvoicesClient({ orders }: { orders: Order[] }) {
  ...
  const handlePrint = (invoiceId: string) => {
    ...
    if (inv) {
      window.open(`/${storeSlug}/dashboard/invoices/${inv.orderId}?print=true`, "_blank");
    }
  };
                ...
                {filtered.map((inv) => (
                  <tr key={inv.id} className="hover:bg-muted/30 transition-colors group">
                    <td className="px-4 py-3">
                      <Link
                        href={`/${storeSlug}/dashboard/invoices/${inv.orderId}`}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Link href={`/${storeSlug}/dashboard/invoices/${inv.orderId}`}>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-muted-foreground hover:text-foreground"
