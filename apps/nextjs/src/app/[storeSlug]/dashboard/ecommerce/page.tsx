...
export default async function EcommercePage({ params }: { params: Promise<{ storeSlug: string }> }) {
      ...
      <div className="space-y-6">
        ...
        >
          <div className="relative z-10 flex flex-wrap items-center justify-between gap-4">
            ...
            <Link href={`/${storeSlug}/dashboard/products`}>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="card-hover p-5">
            ...
            {(() => {
              const t = trendText(totalSalesTrend, "vs last 30 days");
          ...
          <Card className="card-hover p-5">
            ...
            {(() => {
              const t = trendText(totalOrdersTrend, "vs last 30 days");
              return <p className={`mt-1 text-xs ${t.positive ? "text-green-500" : "text-rose-500"}`}>{t.text}</p>;
            })()}
          </Card>
          ...
          <Card className="card-hover p-5">
            ...
            {(() => {
              const t = trendText(aovTrend, "vs last 30 days");
              return <p className={`mt-1 text-xs ${t.positive ? "text-green-500" : "text-rose-500"}`}>{t.text}</p>;
            })()}
          </Card>
          ...
          <Card className="card-hover p-5">
            ...
            <p className="mt-3 text-2xl font-bold tabular-nums text-foreground">{conversionRate.toFixed(2)}%</p>
            {(() => {
              const t = trendText(conversionRateTrend, "from last period");
              return <p className={`mt-1 text-xs ${t.positive ? "text-green-500" : "text-rose-500"}`}>{t.text}</p>;
            })()}
          </Card>
        </div>
          ...
          <Card className="card-hover lg:col-span-2">
            ...
            <CardContent>
              {salesSeries.every((d) => d.total === 0) ? (
                <p className="py-16 text-center text-sm text-muted-foreground">No sales in the last 14 days yet</p>
              ) : (
                <div className="h-[220px] flex items-end justify-between gap-3 pt-4">
                  {(() => {
                    const maxTotal = Math.max(...salesSeries.map((d) => d.total), 1);
                    return salesSeries.map((d, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                        <div
                          className="w-full bg-primary rounded-t-xs hover:opacity-80 transition-all"
                          style={{ height: `${Math.max((d.total / maxTotal) * 180, 4)}px` }}
          ...
          <Card className="card-hover">
            ...
            <CardContent className="space-y-4">
              {inventoryStatus.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted-foreground">All products are well stocked</p>
              ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {promoCodePerformance.length === 0 ? (
                    ...
                    promoCodePerformance.map((c, idx) => (
                      <tr key={idx}>
                        <td className="py-3">
