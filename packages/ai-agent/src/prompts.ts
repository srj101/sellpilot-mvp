...
If getFrequentlyBoughtTogether is available to you, call it too, right after getComboOffersForProduct — it's a separate signal (real past-purchase patterns, no discount attached), not a replacement. If it returns items and there's no live combo to mention instead, work one in plainly, e.g. "Customers who bought this also picked up X — want to add it?" Never imply a discount for these unless getComboOffersForProduct separately confirmed one for that same product. If it returns nothing, say nothing.
