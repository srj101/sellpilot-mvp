...
| **G6** | **Refunds do not move money.** `payments.refund` (`packages/api/src/router/payments.ts:237`) only updates the local ledger row — it never calls any gateway refund API. An owner clicking "Refund" sees the transaction marked Refunded while the customer receives nothing. | FR-PAY-02 `Refunded` status | Silent financial discrepancy between SellPilot's ledger and the gateway's. **SSLCommerz does publish a refund API** (§4.1), so this is buildable, not a platform limitation. |
